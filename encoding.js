function encodeData() {
    ...
    function drawLabels(ctx, levels, canvas) {
        ...
        levels.forEach(level => {
            ctx.fillText(level.label, 5, level.y + 5);  // Adjusting for the baseline of the text
        });
    ...
    nrziCtx.beginPath();
    nrziCtx.lineWidth = 2;
    nrziCtx.strokeStyle = 'red';
    let currentLevel = highLevel; // Assume initial level is high
    ...
    manchesterCtx.strokeStyle = 'purple';
    dataInput.forEach((bit, index) => {
        const x = index * unitWidth + 40;
    ...
    differentialManchesterCtx.strokeStyle = 'orange';
    let lastLevel = highLevel; // Assume initial level is high
